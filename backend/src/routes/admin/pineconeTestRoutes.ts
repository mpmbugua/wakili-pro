 ...
 * @access  Admin only
 */
router.get('/test-connection', authenticateToken, authorizeRoles('ADMIN', 'SUPER_ADMIN'), async (req, res) => {
  try {
 ...
 * @access  Admin only
 */
router.post('/test-upload', authenticateToken, authorizeRoles('ADMIN', 'SUPER_ADMIN'), async (req, res) => {
  try {
